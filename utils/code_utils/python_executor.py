...
#修补
def invoke_llm(scenario, formula, code_snippet, correct, status):
    if 'error' in status.lower() or not eval(correct):
        score = LLM(scenario, formula, code_snippet) *0.5
    else:
        progress_score, outcome_score = LLM(scenario, formula, code_snippet, correct)
        score = 0.5 * progress_score + 0.5 * outcome_score
    return score
...
class PythonExecutor:
    ...
    def eval_prediction(self, scenario, formula:str, code: str, agent_input: str, imports: List[str] = []) -> float:
        if isinstance(imports, np.ndarray):
            imports = imports.tolist()
        if imports:
            code = '\n'.join(imports) + '\n' + code

        code_snippet = EVAL_INPUT_PREDICTION_TEMPLATE.format(code=code, agent_input=agent_input)
        if self.ast_check:
            try:
                ast.parse(code_snippet)
            except:
        ...
        for retry in range(max_retries):
            try:
                correct, status = self.apply(code_snippet, formula)
                #调用judge
                score = invoke_llm(scenario, formula, code_snippet, correct, status)
                return score
