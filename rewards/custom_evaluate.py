...
def get_solution_format_reward(
    solution_str: str,
    extraction_type: str,
) -> float:
    if extraction_type.startswith('answer'):
        formula_match = re.search(r'<formula>(.*?)</formula>', solution_str, re.DOTALL)
        code_match = re.search(r'<code>(.*?)</code>', solution_str, re.DOTALL)
        if formula_match and code_match:
            formula_content = formula_match.group(1)
            code_content = code_match.group(1)
            return 1., {"formula":formula_content, "code":code_content}
        elif formula_match:
            formula_content = formula_match.group(1)
            return 0., {"formula":formula_content, "code":None}
        elif code_match:
            code_content = code_match.group(1)
            return 0., {"formula":None, "code":code_content}
        else:
            return 0., {"formula":None, "code":None}
