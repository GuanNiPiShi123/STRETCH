...
class DatasetManager:
    ...
    def update_seed(self, entries):#有关于seed数据的添加
    ...
    def get_dataset_with_steps(self, name) -> List[Tuple[Dict, int]]:#无用
        if name == 'input':
    ...
    def full_load_data_with_type_counters(self, data: Dict):#加载已有数据保存到dataset manager
        #修补
        """Load datasets"""
...
class ORRayPPOTrainer(ReasonRLRayPPOTrainer):
    #TODO
    #_supported_tasks = {'code_i', 'code_o', 'code_e', 'code_f'}
    def __init__(self, past_epoch_window: int = 10, *args, **kwargs):
        super().__init__(*args, **kwargs)
        #assert self.config.actor_rollout_ref.rollout.n == 1, "OR Agent only supports n=1 for now"
    ...
    def _create_train_code_gen_dataloader(
        ...
        if self.config.azr.gen_data_probabilities_strategy == 'uniform':
        ...
        #修补
        gen_params = {
            'io_data': io_data,#基本只用来提供code snippets的reference
            ...
            'instruction_type': self.config.reward_fn.extraction_type,#answer_conditional
            ...
            #'composite_chance': self.config.azr.data_selection_strategy.composite_chance,#0.0
            'remove_after_return': self.config.azr.reward.generation_reward_config.remove_after_return,#False
            'remove_input_from_snippet': self.config.azr.reward.generation_reward_config.remove_input_from_snippet,#False
            #'include_references': self.config.azr.reward.generation_reward_config.include_references,#True
        }

        get_gen_code_io_data(**gen_params)#真正的数据格式在这里！！！
    ...
    def _create_train_code_pred_dataloader(self, problem_type: str, data_len: int) -> DataLoader:
        ...
        strategy = self.config.azr.pred_data_mix_strategy#max_new
        ...
        get_pred_code_io_data(
            io_data=selected_data,#这个用到了生成的问题
            target_data_len=data_len,
            problem_type=problem_type,
            content_max_length=self.config.azr.data_selection_strategy.content_max_length,
            output_path=parquet_path,
            split='train',
            tokenizer=self.tokenizer,
            instruction_type=self.config.reward_fn.extraction_type,#answer_conditional for prompt template
        )
    ...
    def _compute_batch(self, batch: DataProto, metrics: dict, timing_raw: dict, problem_type: str, executor: PythonExecutor) -> tuple[DataProto, dict]:
        ...
        with marked_timer(f'gen/{problem_type}', timing_raw):
            ...
            #['sequences', 'responses', 'response_length', 'rollout_log_probs']
        ...
        with marked_timer(f'old_log_prob/{problem_type}', timing_raw):
            ...
            loss_agg_mode = self.config.actor_rollout_ref.actor.loss_agg_mode#"token-mean"
        ...
        with marked_timer(f'adv/{problem_type}', timing_raw):
            ...
            if problem_type.startswith('gen'):
                reward_fn_kwargs = {
                    ...
                    'n_samples': self.config.azr.reward.n_samples,#8
    ...
    #TODO 弄清楚里面生成数据的原理
    def _init_seed_dataset(self, problem_types: List[str]) -> List[Dict]:
    # Initialize with seed program using the coordinator
        if ('operation' in problem_types) and ray.get(self.dataset_manager.get_dataset.remote('seed')) == []:
            #修补  随便添加一个seed样本
        ...
        target_size = self.config.azr.data_selection_strategy.data_len * self.config.azr.data_selection_strategy.seed_batch_factor#64*4
    ...
    def fit(self):
        ...
        if self.config.actor_rollout_ref.model.pretrained_tokenizer:
            self.tokenizer.chat_template = "{%- for message in messages -%}{{- '\n' if not loop.first -}}{{- message['content'] -}}{%- endfor -%}"#把角色标记（[USER]、[ASSISTANT]）都去掉，只保留说话内容，每句话之间用换行隔开
        ...
        if self.loaded_datasets:
            ...
            if need_seed_dataset:
                if self.config.azr.seed_dataset is not None:#jsonl格式
                    ...
                    seed_dataset = seed_dataset[:self.config.azr.data_selection_strategy.data_len * 
                                                self.config.azr.data_selection_strategy.seed_batch_factor]#64*4
                    ...
                    if 'code_f' in self.config.azr.problem_types: # we need seed to generate code_f
                        ray.get(self.dataset_manager.update_seed.remote(seed_dataset))#添加seed数据到dataset manager
                else:
            ...
            if need_to_generate_seed:
                sample_problem_types = problem_types #[operation]
                PrettyPrinter.status("DATA", f"Generating missing datasets for {sample_problem_types}...", "info")
                generated_seed = self._init_seed_dataset(problem_types=sample_problem_types)
            ...
            if need_seed_dataset:#继续对seed数据进行处理
        ...
        if self.config.azr.pretrain_pred_steps > 0 and self.global_steps <= self.config.azr.pretrain_pred_steps:
            ...
            self.pretrain_pred = False#选这个

        while self.global_steps < self.total_training_steps:#train_data_len * epoches
            PrettyPrinter.section_header(f"Training Step {self.global_steps}")
            if self.config.azr.data_selection_strategy.composite_scheduler.enabled:#False
                self.scheduler_step()
            ...
            #每训练一步，包括64个样本
            data_len = self.config.data.train_batch_size * self.config.azr.data_selection_strategy.update_iteration #64*1
            if 'code_i' in self.config.azr.problem_types:
            ...
            for _ in range(self.config.azr.data_selection_strategy.update_iteration):
                ...
                with marked_timer('step', timing_raw):
                    ...
                    # concatenate batches
                    batch = DataProto.concat(list(batches.values()))#合并所有生成的数据
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:#True
    ...
    def _load_datasets(self, save_dir: Path):
        ...
        ray.get(self.dataset_manager.full_load_data_with_type_counters.remote(datasets_with_types))#保存已有数据到dataset manager
        PrettyPrinter.status("LOAD", f"Loaded datasets from {self.config.trainer.default_local_dir}", "success")
    ...
    def _load_checkpoint(self):
        ...
        code_dir = Path(self.config.trainer.default_local_dir) / 'OR'
